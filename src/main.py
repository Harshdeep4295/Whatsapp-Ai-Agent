...
# In-memory dedup: track last 200 successfully delivered message IDs to skip Meta duplicate deliveries
_seen_message_ids: set = set()
_seen_order: list = []

def _check_duplicate(msg_id: str) -> bool:
    """Check if msg_id was already processed (without adding it)."""
    if not msg_id:
        return False
    return msg_id in _seen_message_ids

def _mark_processed(msg_id: str) -> None:
    """Mark msg_id as successfully processed after delivery."""
    if not msg_id:
        return
...
async def webhook(request: Request):
    ...
    # Extract message ID for dedup check (but don't mark as processed yet)
    msg_id = None
    try:
        msg_id = body["entry"][0]["changes"][0]["value"]["messages"][0]["id"]
        if _check_duplicate(msg_id):
    ...
    if result:
        ...
        try:
            ...
            # If delivery fails, don't mark as processed so it can retry
            print(f"[webhook] message failed to deliver: {e}")
            raise
    return {"status": "ok"}
